...
st.image('bbot-high-resolution-logo-white-transparent.png', width=200)
st.title("BBOT💰💸💲")
st.markdown("Hello! I'm your friendly chatbot. I can help answer your questions, provide information, or just chat. I'm also super fast! Let's start our conversation!")
...
def main() -> None:
    ...
    #llm_model = st.sidebar.selectbox("Select LLM", options=["Accurate", "Elaborate", "General"])
    ...
    embeddings_model = st.sidebar.selectbox(
        "Select Embeddings",
        options=["nomic-embed-text", "text-embedding-3-smal"],
    ...
    if len(assistant_chat_history) > 0:
        ...
        st.session_state["messages"] = [{"role": "assistant", "content": "HOLA!! how can I help you?☺️."}]
    ...
    if last_message.get("role") == "user":
        ...
        with st.chat_message("tryp"):
            response = ""
            resp_container = st.empty()
            for delta in rag_assistant.run(question):
                response += delta  # type: ignore
                resp_container.markdown(response)
            st.session_state["messages"].append({"role": "tryp", "content": response})
    ...
    if rag_assistant.knowledge_base:
        ...
        input_url = st.sidebar.text_input(
            "Add URL to Knowledge Base - only for paid user🤑", type="default", key=st.session_state["url_scrape_key"]
        )
        ...
        uploaded_file = st.sidebar.file_uploader(
            "Add a PDF -only for paid user💲 :page_facing_up:", type="pdf", key=st.session_state["file_uploader_key"]
        )
